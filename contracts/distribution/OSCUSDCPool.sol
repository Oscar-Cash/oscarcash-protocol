...
contract OSCUSDCPool is USDCWrapper, IRewardDistributionRecipient {
    ...
    modifier checkStart() {
        require(block.timestamp >= starttime, 'OSCUSDCPool: not start');
        _;
    }
    ...
    function stake(uint256 amount)
        ...
        checkStart
    {
        require(amount > 0, 'OSCUSDCPool: Cannot stake 0');
        uint256 newDeposit = deposits[msg.sender].add(amount);
        require(
            newDeposit <= 50000e6,
            'OSCUSDCPool: deposit amount exceeds maximum 20000'
        );
    ...
    function withdraw(uint256 amount)
        ...
        checkStart
    {
        require(amount > 0, 'OSCUSDCPool: Cannot withdraw 0');
